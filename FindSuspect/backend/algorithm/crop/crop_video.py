    ...
    if video_file.endswith(('.mp4', '.avi', '.mov', '.MP4', '.AVI', '.MOV')):  # 비디오 파일 확장자 추가 가능
        ...
        while True:
            ...
            if current_time >= next_target_time:
                ...
                for r in results:
                    ...
                    for box in boxes:
                        if box.cls == 0:  # 0은 'person' 클래스
                            ...
                            # 크롭된 이미지 저장
                            output_filename_cropped = f'{output_folder}/{sys.argv[1]}_frame{int(next_target_time)}_person{person_count}_cropped.jpg'
                            output_filename_cropped2 = f'/data/FindSuspect/backend/src/main/frontend/public/video/{sys.argv[1]}/{sys.argv[1]}_frame{int(next_target_time)}_person{person_count}_cropped.jpg'
                            cv2.imwrite(output_filename_cropped, cropped_human)
                            cv2.imwrite(output_filename_cropped2, cropped_human)
                ...
                #print(f"Processed frame at {next_target_time} seconds")
